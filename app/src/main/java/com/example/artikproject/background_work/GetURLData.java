...
public class GetURLData extends Thread {
    private final String urlq;
    private final Context context;
    /**
     * Класс отвечающий за поиск группы, аудитории, преподователя
     * String - ссылка на сайт
     * Context - активное активити
     */
    public GetURLData(String urlq, Context context){
    ...
    public void run() {
        ...
        try {
            ...
            try{ connection.connect();}
                ...
                new Handler(Looper.getMainLooper()).post(new Runnable() { // Это нужно для вызова вне основного потока
                    @Override
                    public void run() {
                        Toast.makeText(context, "Ошибка при подключении к сайту", Toast.LENGTH_LONG).show();
                    }
                });
                return;
            }
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() { // Это нужно для вызова вне основного потока
            @Override
            public void run() {
                ArrayAdapter<String> adapter = new ArrayAdapter(context, android.R.layout.simple_list_item_1, MainActivity.group_listed);
