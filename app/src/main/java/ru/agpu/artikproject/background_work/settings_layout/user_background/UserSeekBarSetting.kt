...
class UserSeekBarSetting(act: Activity, seekBar: SeekBar, lightSeekBar: Boolean) {
    init {
        if (lightSeekBar) { // Если ползунок светлый
            val level = MySharedPreferences.getPref(act.applicationContext, "light_darker_level", 30) // Получаем сохраненный уровень ползунка (если его нет, то ставим 30%)
            ...
            val level = MySharedPreferences.getPref(act.applicationContext, "dark_darker_level", 30) // Получаем сохраненный уровень ползунка (если его нет, то ставим 30%)
            seekBar.progress = level // Устанавливаем полученый уровень
