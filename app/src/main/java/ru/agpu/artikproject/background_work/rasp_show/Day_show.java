...
public class Day_show {
     ...
     */
    public Day_show(View view) {
        Observable<String> observable = Observable.create(subscriber -> { // Создаем observable, который будет выполняться в отдельном потоке
            ...
            if (r.getCount() != 0) { // Если количество элементов не равно нулю
                ...
                String prev_time = "";
                int prev_number = 0;
                mainText = r.getString(10) + " " + r.getString(11); // Получаем текущий день недели и дату
                do {
                    if (r.getString(4) != null) { // Если название пары не пустое
                        ...
                        if (!para_aud.equals("")) // Если аудитория не пустая
                            para_aud = view.getContext().getString(R.string.Audience) + ": " + para_aud; // Добавляем слово "Аудитория:" к аудитории
                        ...
                        if (para_prepod.contains(",")) { // Если у преподавателя есть "," в тексте
                            ...
                            para_prepod = view.getContext().getString(R.string.Prepod) + ": " + new_para_prepod; // Добавляем слово "Преподаватель:" к аудитории
                        }
                        ...
                        if (!para_group.equals("")) // Если группа не пустая, то добавляем слово "Группа:" к группе
                            para_group = view.getContext().getString(R.string.Group) + ": " + para_group;
                        ...
                        if (Theme.getApplicationTheme(view.getContext()) == AppCompatDelegate.MODE_NIGHT_YES) { // Если стоит ночная тема
                ...
                if (CheckInternetConnection.getState(view.getContext())) { // Проверяем подключение к интернету и обновляем расписание
            ...
            subscriber.onNext(""); // Передаем сообщение о завершении
        });
        ...
        disposable = observable
                ...
                .subscribe(a -> { // Код, выполняющийся после observable
                    try {
                        if (view.isShown()) { // Если фрагмент показан на экране
                            TextView main_Text = view.findViewById(R.id.main_text);
                            main_Text.setText(mainText); // Устанавливаем полученную дату на экране расписания

                            RecyclerView recyclerView = view.findViewById(R.id.day_para_view_rec); // Находим нашу вьюшку на слое
                            recyclerView.setLayoutManager(new LinearLayoutManager(view.getContext())); // Это обязательная фигня, я хз, но без нее список просто не показывается
                            recyclerView.setAdapter(new RecyclerViewAdapter((Activity) view.getContext(), list)); // Устанавливаем новоиспеченный адаптер
                            disposable.dispose(); // Отключаем слушатель observable
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }}
